...
class ClusterHook(Hook):
    ...
    def __init__(self, train_dataloaders, start=None, interval=1, logger=None, cfg=None, epoch_interval=1, **eval_kwargs):
        ...
        self.hdim = 18048
        in_dim = 18048
        self.num_classes = 18048
        self.cls_emb = nn.Embedding(self.num_classes, 256)
        self.net = nn.Sequential(
            nn.Linear(in_dim, self.hdim),
            nn.Tanh(),
            nn.Linear(self.hdim, self.hdim),
            nn.Tanh(),
            nn.Linear(self.hdim, self.num_classes, bias=(True)) 
        )
    ...
    def before_train_epoch(self, runner):
        ...
        for idx, dataset in enumerate(self.datasets):
            ...
            # # y_emb = self.cls_emb(torch.LongTensor(labels))
            # # hin = torch.add(memory_features[0],labels)
            # logit = self.net(torch.FloatTensor(labels))
            # # logit = self.net(torch.FloatTensor(labels))
            # memory_labels.append(logit + start_pid)
